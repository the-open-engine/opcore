...
`run post-edit`, `run pre-commit`, and `run ci` compose the existing evaluators under one resolved configuration. Post-edit compares worktree changes with HEAD and defaults to allowing partial Sense coverage; explicit coverage settings may tighten that behavior. Pre-commit checks staged Verify/native and introduced Sense; CI checks an immutable target, defaulting to HEAD, with an explicit Sense base. Verify and native default to the full `all` comparison for pre-commit and CI. Explicit `--comparison introduced` reuses the exact Git baseline and grandfathers unchanged Fast findings; native findings are subtracted only when the provider produces a comparison-safe baseline, otherwise the workflow is incomplete. See the [Rust-native baseline limitation](providers.md#rust-native). Missing required native prerequisites fail either comparison; configuration never authorizes native execution. `status` and `doctor` inspect the same selected configuration view without evaluating source. `rules --schema` generates the editor schema from the implementation.
...
The `docs` CI job builds the source-generated static site and checks local destinations and HTML anchors. After a successful push-triggered CI run, `docs.yml` rebuilds that exact source and publishes the complete version tree through GitHub Pages at `https://the-open-engine.github.io/opcore/`. Main updates `dev`; a completed release updates its `vX.Y` archive to the newest patch in that minor series. Automatic publication points `stable` at the newest archived minor, so backports update their own archive without moving the default documentation backwards. Pull requests validate the site. The CLI, API, and provider references come from the same source revision as the guides.
...
Default thresholds and hard bounds are versioned policy: the ordinary line threshold is 512 bytes with a 1 MiB hard ceiling; other hard bounds are an 8 MiB NDJSON frame, 4 MiB source file, 10,000 files/request, 256 MiB total source bytes, 256 levels of parser-input structure, a worker pool no larger than available CPUs or 4, 10,000 diagnostics, 8 MiB assessment JSON, 16 MiB cache entry, and 2 GiB cache root. Per-file capability boundaries remain explicit unsupported gaps and local Fast reports them as non-blocking warnings. Truncated required evaluation yields non-clean incomplete status; neither case silently drops evidence.

Recursive parser inputs are bounded before calling parser or AST-fold code. The guard covers delimiter/type nesting, indentation and branch chains, unary/binary/postfix expression chains, nested block operands, and formatted-string interpolation density. This is a conservative resource-safety envelope: exceptionally deep valid source is an explicit unsupported warning, never a process crash or fabricated covered evidence. A local Fast assessment may still be `clean` when its analyzed evidence has no findings.
...
`scripts/docs_versions.py` wraps exact source builds with the version selector and a manifest containing source identity, publisher identity, named routes, and a snapshot digest. It composes the minor archives into a fresh directory, rejects modified snapshots or conflicting source identities for the same release, and preserves existing bytes on retries. Newer patches replace their minor archive; late older patches leave it unchanged. No patch-specific directory or selector entry is published. Static HTML redirects retain existing unversioned links and their query/fragment suffixes. `docs.yml` serializes publication, verifies source ancestry and tag identity, stores the version tree on `gh-pages`, and deploys the complete tree. The workflow runs on main, so the existing Pages environment restriction remains effective. [Documentation versions](versioning.md) defines the URL and recovery contract.

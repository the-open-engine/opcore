...
function unsupportedPlatformMessage(target: string): string {
  ...
  if (target.startsWith("win32-")) {
    return `Unsupported graph-core platform ${target}. Opcore 0.2.2 supports ${supported}; Windows is not supported.`;
  }
  return `Unsupported graph-core platform ${target}. Opcore 0.2.2 supports ${supported}.`;
}

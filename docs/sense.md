...
Machine-output consumers should classify Python absolute imports from `after.resolutionGaps` as `kind: "ambiguous"` and count them in `after.coverage.ambiguousReferences`. They are not counted in `externalReferences`: Sense does not inspect installed packages or resolve an authoritative absolute Python path. This correction changes only the evidence category; status, exit behavior, graph edges, cache identity, and topology are unchanged.

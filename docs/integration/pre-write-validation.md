...
The route runs the same normalized validation path as `lattice validate --request-file`. Overlays are hypothetical: checks read proposed writes and deletes through `ValidationCheckContext.fileView`, and the worktree is not mutated.

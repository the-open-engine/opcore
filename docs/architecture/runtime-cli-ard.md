...
`opcore` is the public product facade for scan/status/check/init/measure/try. Scan, status, check, measure, and try are read-only by default; init is approval-gated setup. `lattice` is the transitional internal work CLI for package-owned graph, inspect, edit, check, validate, status, and doctor command groups; it is shipped in the package for alpha compatibility and command ownership, not as public product branding. Direct old-tool entrypoint execution is unsupported as lattice release behavior, and current external wrappers remain dev validation helpers only.

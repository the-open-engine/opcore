...
The same installed matcher also covers Bash and all MCP tool calls. Read-only calls can trigger a check; the adapter doesn't classify MCP tools by guessed write names. Every invocation evaluates all selected uncommitted worktree changes against HEAD, so payload shapes intentionally receive the same verdict while introduced findings remain. The triggering PostToolUse call has already executed, though a host may replace its result with hook feedback. Committing moves the comparison baseline rather than repairing findings. Doctor inspects owned hook configuration and receipts without changing them, but host trust and session activation remain unverified until a host-level edit smoke test observes execution.

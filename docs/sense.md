...
|---|---|---|
| Node | Static imports and re-exports; guarded direct top-level `.cjs` `require("./target.cjs")` calls; runtime and type-only remain distinct | Bare packages, aliases, other `require` forms, dynamic imports, query/hash suffixes, or multiple extension/index candidates |
| Python | `from .module import name`, `from . import name`, and one-component level-zero imports with one unambiguous sibling `.py`, `.pyi`, or package target | Dotted absolute imports, non-sibling resolution, sys.path, namespace/config ambiguity, missing targets, or multiple sibling candidates |
...
Coverage counts recognized `node:` built-in boundaries as a subset of external references while retaining the existing external total; resolved, ambiguous, unresolved, unsupported-loader/dynamic or conditional, parser-failed, configuration-error, and truncated counters remain separate. Opcore uses a pinned, conservative list of public Node built-ins and subpaths rather than trusting the prefix alone, so unknown `node:` names remain ordinary external gaps. Each graph view also returns a deterministic sample of at most five `resolutionGaps` entries containing the path, import specifier, and gap kind; `resolutionGapsTruncated` marks a larger set. Sense confirms a CommonJS runtime edge only for a direct call in a top-level variable initializer or expression statement of a `.cjs` file, with one literal relative `.cjs` specifier and no binding or reassignment of `require` anywhere in the file. Bare Node specifiers, unresolved or dotted Python absolute imports, aliases and every other `require` form, dynamic imports, unsupported Rust project context, external or conditionally selected Go imports, HCL/Shell/Protobuf semantic extraction, and recognized extension-classified unsupported source make the report `partial`; malformed relevant `go.mod` is `incomplete`. Python sibling resolution is intentionally limited to one-component imports in the importer's directory; it never searches repository roots, parent packages, installed packages, project configuration, or `sys.path`. None can silently support a globally clean graph claim. `--allow-node-builtins` narrowly acknowledges recognized explicit `node:` boundaries while preserving their evidence; it does not accept unknown `node:` names, bare packages, missing local imports, aliases, or any other partial cause. Rust facts are structural rather than runtime: they inform dependency breadth, reverse impact, and importance but never runtime-cycle findings. Go package imports are runtime edges. Unknown docs/data remain outside the source-candidate envelope. Confirmed cycles remain sound findings under partial coverage.

Machine-output consumers see a one-component Python absolute import as a runtime edge only when the importer's directory contains one unique `.py`, `.pyi`, or package target. Missing, dotted, and colliding absolute imports remain `kind: "ambiguous"` in each view's `resolutionGaps` and count in `coverage.ambiguousReferences`, not `externalReferences`. Sense does not inspect parent roots, installed packages, project configuration, or `sys.path`, so it never guesses an edge outside that exact sibling case.

Dependency facts are bounded per file and in aggregate. Overflow is explicit coverage loss, never silent truncation. Duplicate-analysis limit issues distinguish callable extraction, token-region extraction, request collection, and exact token comparison. For extraction and request-collection limits, machine-readable JSON retains the complete deterministic affected-path list within the 10,000-file capture and 8 MiB assessment bounds; human output shows five paths and directs callers to `--json` for the complete bounded evidence. Exact token-comparison exhaustion stops evaluation and therefore reports a bounded deterministic path sample with explicit truncation instead of claiming to know paths from unchecked candidates. If complete evidence cannot fit the assessment bound, Sense uses its explicit output-truncation/incomplete failure instead of presenting a sample as complete. Each issue also reports its fixed limit, processed count, applicable before/after view, and an honest next step; no runtime option is presented as raising a compiled safety bound. Topology and report algorithms are iterative, deterministic, and bounded; graph traversals are linear while ordered indexes add logarithmic factors.

`effectivePolicy.importantFanIn` is the configured direct-dependent threshold for mechanically important modules; it does not turn unresolved dependencies into confirmed fan-in. `dedup_region_file_limit` means one file exceeded the fixed 4,096 token-region-anchor bound. For generated or vendored trees, use literal repository-relative `targets.exclude` entries as described in [Select targets](configuration.md#select-targets); exclusions are not globs and should not hide maintained source. In JSON output, `documentationCoverage.evaluated: false` normally means no qualifying newly-important or authoritative public-surface change required documentation evaluation. A `beforeRegistry` or `afterRegistry` value of `not_read` means the registry was not needed for that view, not that configuration was ignored. `publicSurfaceAuthoritative: false` means the parser could not establish a complete explicit public surface, so Sense does not claim an authoritative surface comparison. If bounded important-node observations prevent documentation evaluation, the report is `incomplete` and an issue explains that exception.

...
describe("Opcore JSON schema wire constraints", () => {
  ...
  it("accepts command-router manifest and result schemas", () => {
        ...
        capabilities: {
          ...
          graph: {
            ...
            nativeArtifacts: validManagedToolDescriptor().capabilities.graph.nativeArtifacts.map((artifact) => ({
              ...artifact,
              packageName: artifact.bundledPackageName,
    ...
    assert.equal(isValidDefinition("OpcoreRuntimeInfoPayload", validOpcoreRuntimeInfo()), true);
    assert.equal(isValidDefinition("OpcoreDoctorPayload", validOpcoreDoctor()), true);
    assert.equal(
      isValidDefinition("CommandRouterResult", {
        ...validRouterResult(),
        bin: "opcore",
      ...
      isValidDefinition("GraphReleaseReceipt", {
        ...
        commandCoverage: receipt.commandCoverage.map((entry) =>
          entry.id === "opcore-graph-build"
            ? { ...entry, command: ["build"], canonicalCommand: ["opcore", "build"] }
            : entry
        )
      }),
      false
  ...
  it("accepts and rejects release receipt schemas", () => {
    ...
    assert.equal(
      isValidDefinition("ReleaseReceipt", {
        ...receipt,
        packageNames: [...receipt.packageNames, "@the-open-engine/opcore-graph"],
        packages: [
          ...receipt.packages,
          {
            ...receipt.packages[0],
            ...
            manifest: { ...receipt.packages[0].manifest, name: "@the-open-engine/opcore-graph" },
            nativeArtifacts: []
          }
        ]
      }),
    ...
    assert.equal(
      isValidDefinition("ReleaseReceipt", {
        ...receipt,
        nativeArtifacts: receipt.nativeArtifacts.map((entry) => ({
          ...entry,
          packageName: entry.bundledPackageName
        }))
      }),
      false
    );
  ...
  it("accepts and rejects ASP dogfood receipt schemas", () => {
    ...
    assert.equal(
      isValidDefinition("AspDogfoodReceipt", {
        ...receipt,
        provider: { ...receipt.provider, command: ["opcore", "asp", "serve"] }
      }),
      false
    );
    assert.equal(
...
function validGraphReleaseReceipt() {
  ...
  return {
    ...
    rustCommandCoverage: rustCommandIds.map((id) => {

...
- TypeScript CLI router owns transitional internal `lattice` command dispatch and composes package-owned graph, edit, check, and validate adapters.
...
`opcore` is the public product facade for scan/status/check/init/measure/try. Scan, status, check, and measure are read-only against the target repo; try writes only generated local sample repos; init is approval-gated setup. `lattice` is the transitional internal work-router bin for package-owned graph, inspect, edit, check, validate, status, and doctor command groups; it is shipped for alpha compatibility and command ownership, not public product branding. Direct old-tool entrypoint execution is unsupported as lattice release behavior, and current external wrappers remain dev validation helpers only.
...
| `opcore init` | runtime | Run a read-only scan first, print coverage before findings, present approval-gated additive repo/agent setup, then write `.opcore/config`, delimited guidance, a managed `.opcore/` `.gitignore` line in Git repos, optional hooks, and undo metadata only when approved. |
...
## Internal Dual-Bin Split

The `@the-open-engine/opcore` package intentionally ships two bin entries. `opcore` points to `dist/index.js` and owns the public product facade routes: scan, status, check, init, measure, and try. `lattice` points to `dist/lattice/index.js` and owns the internal work-router groups: graph, inspect, edit, check, validate, status, and doctor.

The parallel source trees under `packages/opcore/src/` and `packages/opcore/src/lattice/` are not duplicate-router drift. `graph-provider-client.ts` exposes product-facade `opcoreGraph*` helpers, while `lattice/graph-provider-client.ts` exposes `cliGraph*` helpers and adds `cliGraphSearch` for inspect/edit routing. `validation-composition.ts` exposes the product-facade validation runner and check adapter, while `lattice/validation-composition.ts` exposes `validateCommandAdapter` and `editValidationRunner` for work-router flows. Both trees remain live because the two bins have different command and composition needs.

Both trees intentionally retain the provider id `lattice-graph` as a transitional internal contract/artifact name. `lattice`, `lattice-graph`, and related implementation paths are launch-naming allowlist entries for internal compatibility and generated evidence only. They must not be introduced as public Opcore product branding.

Generated release, cutover, and ASP dogfood receipts that still contain `@the-open-engine/lattice-*` or `packages/cli` remain hand-offs to #2 identity rename and #6 release regeneration. The stale `CONTRIBUTING.md` package-ownership list is also a #2 hand-off. Do not hand-edit those generated receipts or the contributing ownership list as part of router-split documentation.

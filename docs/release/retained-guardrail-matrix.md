...
- `docs/release/cutover-receipt.json` (#30): installed `node_modules/.bin/opcore` command receipts, with `opcoreBinOnly:true` meaning only Opcore-owned bins are exposed and old bins are absent from the cutover environment.

...
- Replaces Python grammar heuristics with exact-interpreter `compile()` validation over `.py`/`.pyi` after-state content, including structured ranges, interpreter provenance, and fail-closed tool outcomes.

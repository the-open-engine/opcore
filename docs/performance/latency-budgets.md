...
Default `npm run latency:check` uses synthetic passing fixture telemetry from `tests/fixtures/latency/telemetry-pass.jsonl` so CI has a deterministic trend signal before #35 adds Docker E2E benchmark evidence. The default mode is non-blocking: over-budget evidence is reported but exits zero. Use `--fail-on-over` only when promoting the gate to a blocking regression check.

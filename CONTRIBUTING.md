...
Set the Cargo package and lockfile version, merge the reviewed change after CI passes, then push the matching stable `vX.Y.Z` tag. CI builds both platforms, stages matching npm metadata and checksums, and tests the packed artifact. It publishes GitHub assets before exercising the packed install against those assets and publishing that exact npm tarball. If publication fails after the GitHub Release exists, preserve its assets and diagnose the failed step before retrying; the job refuses to replace an existing release.

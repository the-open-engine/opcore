    ...
    for (const file of packageSourceFiles()) {
      ...
      for (const source of imports(file)) {
        ...
        if (packageDir === "validation-python") {
          assert.equal(
            ["@the-open-engine/opcore-contracts", "@the-open-engine/opcore-validation", "jsonc-parser", "smol-toml"].includes(source),
            true,
            `${file} imports ${source}`
          );
        }
        if (packageDir === "validation-clone") {
          assert.equal(

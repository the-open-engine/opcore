...
The `docs` CI job builds the source-generated static site and checks local destinations and HTML anchors. After all CI gates pass on `main`, `deploy-docs` publishes that same artifact through GitHub Pages at `https://the-open-engine.github.io/opcore/`. Pull requests and release tags validate the site without deploying it. The CLI, API, and provider references come from the same source revision as the guides.

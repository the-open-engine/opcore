...
| `opcore init` | runtime | Run a read-only scan first, print coverage before findings, present approval-gated additive repo/global agent setup, then write scoped config/guidance, Claude Code and Codex write-gate wiring, managed `.opcore/` ignore state, optional pre-commit hook scripts, and undo metadata only when approved. |
...
The `@the-open-engine/opcore` package intentionally ships one public bin: `opcore` points to `dist/index.js`. The root router owns the product facade routes: scan, status, check, init, measure, and try. It also dispatches advanced work-router groups through the same public bin: graph, inspect, edit, check, validate, status, and doctor.
